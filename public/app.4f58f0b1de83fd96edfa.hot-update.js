...
const main = async err => {
  ...
  if (data) {
    ...
    const search = () => {
      ...
      data.querySelector('.yes').style.display = 'none';
      data.querySelector('.no').style.display = 'none';
      setTimeout(() => {
        if (postcode.value || city.value) {
          ...
          fetch('/wp-admin/admin-ajax.php', {
            ...
            if (data.postcode || data.city) {
              data.querySelector('.yes').style.display = 'block';
            } else {
              data.querySelector('.no').style.display = 'block';
            }
          }).catch(error => {
            console.error(error);
          });
        }
      }, 300); //   diet.forEach(value => {
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "3f6805d48f0a64b1e594"; }
/******/ }();
/******/ 
/******/ }
);
//# sourceMappingURL=app.4f58f0b1de83fd96edfa.hot-update.js.map